...
public abstract class TSCodecAC3
{
    ...
    public static void Scan(TSAudioStream stream, TSStreamBuffer buffer, ref string tag)
    {
        ...
        {
            stream.AudioMode = channelMode switch
            {
                0 => // 1+1
        ...
        stream.SampleRate = srCode switch
        {
            0 => 48000,
            1 => 44100,
            2 => 32000,
        ...
        if (stream.StreamType != TSStreamType.AC3_PLUS_SECONDARY_AUDIO)
        {
            ...
            {
                case TSStreamType.AC3_PLUS_AUDIO when bsid == 6:
                case TSStreamType.AC3_AUDIO:
                    stream.DialNorm = (int)dialNorm * -1;
                    break;
                case TSStreamType.AC3_PLUS_AUDIO when secondFrame:
    ...
    {
        var sampleOffsetE = buffer.ReadBool();
        if (sampleOffsetE)
            buffer.BSSkipBits(12);
        ...
        if (buffer.ReadBool()) return; //discard_unknown_payload

        buffer.BSSkipBits(1);

        if (sampleOffsetE) return;
